...
import { FitnessGoal, BodyStats, GoalType } from '../types';
...
export default function Goals() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Fitness Goals</h1>
          ...
          <div className="space-y-3">
            ...
            {activeGoal.target_weight && (
              <div className="flex justify-between items-center">
                ...
                <span className="font-semibold">{activeGoal.target_weight} {profile?.unit_preference === 'metric' ? 'kg' : 'lbs'}</span>
              </div>
            )}
            <div className="flex justify-between items-center">
              <span className="text-indigo-100">Target Calories:</span>
              <span className="font-semibold">{activeGoal.target_calories} cal/day</span>
            </div>
            <div className="grid grid-cols-3 gap-4 mt-4 pt-4 border-t border-indigo-400">
              <div>
                <p className="text-indigo-100 text-sm">Protein</p>
                <p className="text-2xl font-bold">{activeGoal.target_protein}g</p>
              </div>
              <div>
      ...
      {!showForm ? (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Target Weight (optional) - {profile?.unit_preference === 'metric' ? 'kg' : 'lbs'}
              </label>
              <input
                type="number"
                step="0.1"
            ...
            {formData.target_calories && (
              <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-3 gap-4">
                  {(() => {
                    const macros = calculateMacros(parseInt(formData.target_calories), formData.goal_type);
                    return (
                      <>
                        <div>
